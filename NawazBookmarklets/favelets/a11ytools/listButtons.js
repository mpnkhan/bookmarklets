	...
	function listButtons() {
		...
		Array.from(btns).forEach(function(button) {
			...
			if (button.getAttribute('aria-label')) {
				...
				notes += '- button gets accessible name from <code>aria-label</code>. Check that the accessible name does not contradict the text on screen<br>';
				warn = true;
			...
			if (button.getAttribute('aria-labelledby')) {
				...
				if (sources.length > 1) {
					...
					notes += '- button gets accessible name from <code>aria-labelledby</code> (multiple sources). Check that the accessible name does not contradict the text on screen<br>';
					warn = true;
				} else {
					...
					notes += '- button gets accessible name from <code>aria-labelledby</code> (single source). Check that the accessible name does not contradict the text on screen<br>';
					warn = true;
				}
				if (accName.trim().toLowerCase().indexOf(tc.trim().toLowerCase()) === -1) {
			...
			if (isHidden(button)) {
				...
				if (isHidden(button)) {
					...
					notes += 'Button *was* hidden but has been temporarily revealed on the page<br>';
				}
			}
			...
			if (button.getAttribute('title') && button.getAttribute('title') !== tc) {
				notes += '- button has a <code>title</code> which is different from text content.';
				if (accName !== '') {
					notes += 'This <code>title</code> content -- "' + button.getAttribute('title') + '" -- will not be perceivable to assistive tech, keyboard and touch screen users<br>';
				}
				notes += '<br>';
				warn = true;
			}
			...
			if (button.getAttribute('title') && button.getAttribute('title') === tc) {
				notes += '- button has a <code>title</code> which is the same as the text content and therefore adds no extra useful information/context. This <code>title</code> content -- "' + button.getAttribute('title') + '" -- will not be perceivable to assistive tech, keyboard and touch screen users<br>';
				warn = true;
			}

			if (button.tagName === 'BUTTON') {

				if (button.getAttribute('tabindex') && button.getAttribute('tabindex') !== '-1') {
					notes += 'Button has a <code>tabindex</code> but it is not needed because the <code>button</code> is natively focusable<br>';
				}

				if (button.getAttribute('tabindex') && ((button.getAttribute('tabindex') !==
						'-1') && (button.getAttribute('tabindex') !==
						'0'))) {
					notes += '- Button has a positive <code>tabindex</code> (not -1 or 0). Could cause a <a href="https://www.w3.org/TR/WCAG21/#focus-order" target="_blank">2.4.3 Focus order</a> failure.<br>';
					warn = true;
				}

				if (button.getAttribute('role') === 'button') {
					notes += 'Button has a <code>role</code> of <code>button</code>. Not needed as it is a <code>button</code> element that is a button by default<br>';
				}

				if ((button.getAttribute('aria-hidden') !== 'true') && (button.getAttribute('tabindex') === '-1')) {
					notes += '- <code>&lt;button&gt;</code> has a negative <code>tabindex</code> which means it will not be keyboard-operable<br>';
					warn = true;
				}

				if ((button.getAttribute('aria-hidden') === 'true') && (button.getAttribute('tabindex') !== '-1')) {
					notes += '- <code>&lt;button&gt;</code> has <code>aria-hidden="true"</code> but can still be focused by keyboard user as it does not have a negative <code>tabindex</code><br>';
					warn = true;
				}

				if ((button.getAttribute('role')) && (button.getAttribute('role') !== 'button')) {
					notes += '- <code>&lt;button&gt;</code> has a <code>role</code> set that is not <code>button</code>. It is set to "' + button.getAttribute('role') + '". This overrides the native role and will likely confused assistive tech users<br>';
					warn = true;
				}
			}
			if (button.tagName === 'A') {
				if ((!button.getAttribute('href')) && (!button.getAttribute('tabindex'))) {
					notes += '- button is based on an <code>&lt;a&gt;</code> element, does not have an <code>href</code> or <code>tabindex</code>,  and therefore is not focusable<br>';
					warn = true;
				}

				if (!button.getAttribute('href') && button.getAttribute('tabindex') && button.getAttribute('tabindex') !== '-1') {
					notes += '- button is based on an <code>&lt;a&gt;</code> element, does not have an <code>href</code>, but is focusable because it has been provided with a positive <code>tabindex</code><br>';
					warn = true;
				}

				if (button.getAttribute('href') && button.getAttribute('tabindex') && button.getAttribute('tabindex') !== '-1') {
					notes += '- button is based on an <code>&lt;a&gt;</code>. It is focusable because there is a <code>tabindex</code> present<br>';
					warn = true;
				}
			...
			if ((button.tagName !== 'BUTTON') && (button.tagName !== 'A') && (button.getAttribute('role') === 'button')) {
				notes += '- This has a <code>role</code> of <code>button</code> but is not a <code>button</code> element.';
				if (button.getAttribute("tabindex") === "-1") {
					notes += '<br>- <strong>Note</strong>: this button has a negative <code>tabindex</code> and will not be keyboard-focusable.';
				} else {
					notes += 'Check that it is keyboard-operable (should activate with <kbd>Enter</kbd> and <kbd>Space</kbd> key)<br>';
				}
				warn = true;
			}
			...
			*/
			...
			row += '<td>';
			if (warn) {
				row += '<div class="issues">Possible issue(s) found with this button</div>';
			}
			if (err) {
				row += '<div class="issues">Definite issue(s) found with this button</div>';
			}
			consoleOutput = 'Button \'' + tc.trim() + '\':\n' + notes + 'Markup with issue:\n' + snippet + '\n---------------\n';
			...
			row += '<td><button data-button-ref="' + i + '" class="highlightButton" type="button" aria-pressed="false" aria-label="Highlight this issue on the page visually">Show</button></td>';
			row += '</tr>';
			i++;
		...
		const popUpWinButtons = window.open('', 'popUpWinButtons', 'height=800,width=1000');
