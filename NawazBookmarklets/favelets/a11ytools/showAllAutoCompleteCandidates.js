javascript: (function() {
	function showAllAutoCompleteCandidates() {
		...
		o.innerHTML = ".auto-complete-candidate {outline:3px dotted orange;}.auto-complete-required {outline:3px solid red;outline-offset:2px;box-shadow:0 0 0 8px white}", document.querySelector("head").append(o);
		const e = document.querySelectorAll("input:not([autocomplete]):not([type=radio]):not([type=checkbox]):not([type=hidden]):not([type=submit]),select:not([autocomplete])");
		console.clear();
		let n = "",
			t = "",
			a = "",
		...
		Array.from(d).forEach(function(o) {
			o.addEventListener("change", function() {
				...
				Array.from(e).forEach(function(o) {
					...
					"\n", a = o.outerHTML "\n"
				}), console.clear(), a = (a = (a = a.split('class="amended"').join("").split(" amended").join("").split(' class=""').join("")).split("auto-complete-candidate auto-complete-required").join("")).split(' class=""').join(""), console.log("The following "
					p " fields require an autocomplete attribute as specified:\n\n"
					t "\nFull details:\n\n{code}\n"
					a "{code}")
			})
		});
		const s = document.createElement("button");
		s.setAttribute("type", "button"), s.innerHTML = "Prep for screenshot!", s.style = "position:fixed;z-index:10000;top:1em;right:1em;background:red;color:white;font-weight:bold;", document.querySelector("body").append(s), s.addEventListener("click", function() {
			s.remove();
